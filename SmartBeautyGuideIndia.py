...
import phonenumbers
import re
fp=open("Smartbeautyguide-India.csv","w")
docterNameArray=[]
url="https://www.smartbeautyguide.com/select-surgeon/?location=india&search-type=surgeon-locator&search=india"
...
def getAllPageData(data):
	for content in data:
		...
		if(content.find("a").has_attr('href')):
			...
			website="-"
		tel=phonenumbers.format_number(phonenumbers.parse(phoneNumber, 'IN'),'IN')
		tel='+91'+tel
