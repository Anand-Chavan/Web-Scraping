...
import phonenumbers
import re
fp=open("Smartbeautyguide-UAE.csv","w")
docterNameArray=[]
url="https://www.smartbeautyguide.com/select-surgeon/?location=UAE&search-type=surgeon-locator&search=UAE"
...
def getAllPageData(data):
	for content in data:
		...
		if(content.find("a").has_attr('href')):
			...
			website="-"
		tel=phonenumbers.format_number(phonenumbers.parse(phoneNumber, 'AE'),'AE')
		tel='+971'+tel
