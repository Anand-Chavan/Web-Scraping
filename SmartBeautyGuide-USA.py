...
import phonenumbers
import re
fp=open("Smartbeautyguide-USA.csv","w")
docterNameArray=[]
url="https://www.smartbeautyguide.com/select-surgeon/?page=2&location=usa&search-type=surgeon-locator&search=usa"
...
def getAllPageData(data):
	for content in data:
		...
		if(content.find("a").has_attr('href')):
			...
			website="-"
		telephone=re.sub('[^0-9]+', '',phoneNumber)
		print(telephone)
		tel=""
		if(telephone!=""):
			tel=phonenumbers.format_number(phonenumbers.parse(telephone, 'IN'),'IN')
			tel='+1'+tel
